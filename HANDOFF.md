...
- آمنة بلا الـSQL (allSettled)، لكن للوظائف الكاملة (التكلفة/الإسناد/التسجيل اليومي/الالتزام) **شغّل** `campaigns_tracking.sql`.

> 🔴 إجراء وحيد عالق منك: تشغيل `src/modules/campaigns/supabase/campaigns_tracking.sql` لتفعيل ميزات التكلفة/الإسناد/التسجيل اليومي في الحملات.
