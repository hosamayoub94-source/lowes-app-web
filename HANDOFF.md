...
- **⏳ يحتاج تطبيقك (لا يملك Claude صلاحية):** (1) **تدوير المفاتيح** (service_role + توكنات الجداول `LOWES-SYRIA/TURKEY-2026` + `LOWES-META-SYNC-2026`). (2) تطبيق **`supabase/migrations/0007_audit_remediation.sql`** بالـSQL Editor (RPC مخزون ذرّي #5/#15 + عمود `yurtici_exported_at` #16) ثم تعديل `warehouseService.js`/`OrdersScreen` لاستخدامهما (موثّق). (3) نشر تصحيحات Edge للمصادقة الخادمية (#3 manage-employee · #10 ai-assistant · #20 track-yurtici · إزالة توكنات افتراضية #11/#19/#2/#12) بعد ضبط الأسرار. (4) (اختياري) عكس افتراضي mock (§5 بالوثيقة). **كله مفصّل بخطوات في `docs/SECURITY-REMEDIATION.md`؛ المرجع الكامل للأخطاء الـ39 في `docs/audit-findings-full.txt`.**
