...
### 🗓️ جلسة 16 يونيو 2026 — المحاسبة قسمان: «المصاريف والشحن» + «المالية العامة» (commit 2b28832 على فرع worktree — لم يُدمج بعد على main)
طلب المالك: تقسيم المحاسبة قسمين بتسميات أوضح + رؤية الوارد/الصادر الشهري لكل **شركة شحن/جهة** + إضافة مصدر مصروف + حصر «الحساب المركزي» بالأدمن.
- **القسم 1 «المصاريف والشحن» 🚚** (`/accounting` · `AccountingScreen`) — لفادي ووسيم (دورهم **مدير** فعلياً، لا محاسب → وصولهم يعمل بلا تغيير RLS): جدول **الوارد/الصادر/الصافي لكل جهة شهرياً** (مكوّن مشترك `SourceBreakdown` + منطق نقيّ `sourceBreakdown.logic.js`) + حقل **«الجهة/المصدر» نصّ حر مع datalist** = إضافة مصدر مصروف بمجرّد كتابته (لا كتابة على جدول `accounting_categories` المحصور بالأدمن — المصدر يُخزَّن بحقل `category` الحر). السماح للمدير/المحاسب/مدير المبيعات بإدخال القيود (`canEnter`)، الحذف للأدمن/المدير فقط.
- **القسم 2 «المالية العامة» 🏦** (`/ledger` · `AccountingDashboard`) = **الحساب المركزي، أدمن فقط** (قرار المالك «هو القسم الثاني نفسه»): nav بلا `perm` + مسار `[ADMIN]` (كان FINANCE_ROLES) → المديرون لم يعودوا يرونه. + فلتر **«الجهة/المكان»** + نفس جدول الوارد/الصادر الشهري.
- **التحقّق:** `npm run build` أخضر ×2 + تست منطق `test-source-breakdown.mjs` **12/12** (الترتيب حسب **عدد الحركات** لا حجم العملة — الـSYP بأرقام ضخمة لا يطغى) + إقلاع نظيف بلا أخطاء console. (تعذّر تحقّق بصري مصادَق: PIN الأدمن غير معروف بثقة، ولم أخمّن.)
- **اكتشاف:** «PIN الأدمن 6311» القديم بالذاكرة **مرفوض لحساب hosam ayoub** (راجع `[[face_manager_social]]`).
- **🔜 للنشر:** دمج الفرع على `main` (Vercel/CI) ثم تحقّق حيّ. ملف `supabase/accounting_accountant_access.sql` **اختياري** (لمحاسب مستقبلي فقط — فادي ووسيم مديرون فلا يلزم). مرجع: `[[accounting_two_sections]]`.
