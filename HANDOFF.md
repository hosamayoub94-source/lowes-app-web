...
### 🗓️ جلسة 16 يونيو 2026 (UAT المحاسبة) — تنفيذ punch-list اختبار القبول الـ11 (فرع `claude/condescending-jones-32992d` · بانتظار دمج المالك)
المرجع: `docs/accounting-uat-findings-2026-06-16.md`. نُفِّذت كل البنود الـ11:
- **🔴 1 — منع ازدواج التسليم:** حُذف `OP_CAT.HANDOVER/SUPPLY` من `OP_EXPENSE_SOURCES/OP_INCOME_SOURCES` في `operationalAccount.js` (بقيا تسميتين للتحويلات فقط). التسليم حصراً عبر زرّ «رفع/تسليم الرصيد» (`createTransfer`).
- **🔴 2 — شفافية سعر الصرف:** `ChannelPnL.jsx` يعرض السعر+تاريخه بالتذييل (`rateMeta`) + مودال `RateModal` لتحديث USD→TRY/SYP عبر `upsertExchangeRate` (من payrollService).
- **🟡 3 — تبويب «🔄 تحويلات/تسليمات»** على `/accounting` يعرض قيود `transfer` للكتاب التشغيلي (أثر تدقيقي).
- **🟡 4 — كشف حساب تراكمي:** `channelStatement.logic.js` (نقيّ) + `ChannelStatement.jsx` (مودال «📒 كشف حساب») — كل الحركات + رصيد جارٍ لكل عملة، مستقلّ عن الشهر.
- **🟡 5 — فلترة فترة مخصّصة:** زرّ «🗓️ مدى مخصّص» يبدّل بين شهر واحد ومدى تواريخ (`inPeriod`/`periodLabel`) يغذّي الجدول/التقرير/ChannelPnL/التصدير.
- **🟡 6 — أداة ربط القيود القديمة:** `ChannelRemapTool.jsx` (أدمن فقط، زرّ «🔗 ربط بالقنوات») — تجمّع القيود بلا `channel_id` حسب `category` وتعيّن قناةً لكلٍّ بـ`updateEntry` (تأكيد صريح قبل التعديل).
- **🟢 7+8 — مودال التسليم:** اختيار **المحفظة** (بنك/شام/كاش — `createTransfer` يقبل `wallet`) + عرض الرصيد الحالي + زرّ «سلّم الكل».
- **🟢 9 — إبراز منتقي القناة** بنموذج القيد (إطار teal + وسم «مُستحسَن»).
- **🟢 10 — توضيح:** عنوان التقرير = «التدفّق النقدي لكل قناة» (ليس هامش الربح؛ الأخير في `/profitability`).
- **🟢 11 — توضيح تسوية الخزائن** مع الرصيد التشغيلي (سطر تحت `TreasuryPanel`).
- **التحقّق:** `npm run build` أخضر ×متكرر + اختبارات نقيّة: accounting-books 9/9 · channel-pnl 10/10 · source-breakdown 16/16 · **channel-statement 10/10 (جديد)**. لم يُجرَ تحقّق بصري (login بـPIN). مرجع: `[[accounting-channels-books]]`.
