...
### 🗓️ جلسة 18 يونيو 2026 — فحص شامل + تست كامل (audit + fixes) ✅ كود مُصلَح ومُتحقَّق حيّاً (worktree — لم يُنشر) · ⏳ بنود قاعدة/أمان على المالك
طلب المالك: «فحص شامل للنظام وتست كامل لكل التطبيق» + «إصلاح كل شي شامل القاعدة» + «تست حيّ». نُفِّذ عبر فحص ثابت متعدّد الوكلاء (find→adversarial-verify) + تدقيق قاعدة حيّة + إصلاحات + smoke حيّ.

- **النتيجة:** 48 اكتشافاً → **39 مؤكَّداً** (3 critical · 10 high · 11 med · 15 low) بعد تحقّق عدائي (8 رُفضت كإيجابيات كاذبة، مثل `createTransfer` **ذرّي فعلاً** عبر insert واحد). التحقّق النهائي: `npm run build` أخضر · **lint 0 errors** (كان 117؛ 225 warnings متبقّية = unused-vars + exhaustive-deps، مؤجّلة عمداً لأن exhaustive-deps خطِر آلياً) · **الاختبارات 7/7** (account-statement صار 23/23 باختبار انحدار جديد) · smoke حيّ بجلسة admin (محقونة عبر `lowes_manual_session`).
- **🔴 الأخطر (يحتاجك أنت — لا يقدر Claude):** مفتاح **`service_role`** كان مكتوباً نصّاً في `supabase/functions/{sheet-to-app,sync-meta-insights}/index.ts` + `scripts/setup-yurtici-cron.sql` → بتاريخ git = **مخترَق، دوّره فوراً** من Supabase Dashboard. أُزيل من الكود. التفاصيل + باقي بنود المالك في **`docs/SECURITY-REMEDIATION.md`**.
- **معماري (مؤكَّد حيّاً):** RLS مفتوح (`USING(true)`) + مفتاح anon العام = قراءة/كتابة كل البيانات تقريباً عبر المفتاح المشحون بالـbundle (يشمل إعادة تعيين PIN = اختطاف حساب). التطبيق **يعتمد** عليه → يحتاج خطة مرحلية (نقل الكتابات الحسّاسة لـEdge Functions + JWT حقيقي للكل + تشديد RLS تدريجي). لا يُصلَح بسطر واحد. راجع الوثيقة §3.
- **إصلاحات كود مُطبَّقة ومُتحقَّقة (worktree):** TaskCard hook (#31) · DevToolbar hook (#32) · تحويل محفظة ذرّي + transfer_group (#4/#25) · await للمخزون بالترحيل الجماعي (#6) · وردية ليلية تُغلق (#9) · store كشف كامل + استثناء التحويلات (#14/#26) · عملة ميتا/ROAS (#18) · require()→import + DEV-gate لتتبّع المؤقّتات (#21/#35) · حارس موافقة الإجازة (#22) · هامش الربحية عند تكلفة=0 (#23) · اشتراك التفاعلات محصور بالغرفة + تنظيف مؤقّتات الكتابة (#29/#33) · مؤقّتات التوست (#34) · سلسلة المستلمين بالأتمتة (#36) · أحدث 50 مهمة بالطابور (#37) · رواتب NaN (#39) · ترتيب نفس-اليوم بكشف الحساب +TDD (#27) · توقيت الحضور المحلي (#38) · توثيق خروج الجلسة اليدوية + رسالة «معطّل» (#7/#28) · **تفعيل طبقة الإنتاج الخاملة** (errorReporter/health/offline/realtime — #13، تأكّد إقلاعها حيّاً) · **`verify-pin` يحجب المعطّلين** (#7) · حذف مفتاح service_role من الكود.
- **✅ مُنجَز لاحقاً (18 يونيو، عبر جلسة Supabase بالمتصفّح):** هجرة **`0007` طُبِّقت ومُتحقَّقة على القاعدة الحيّة** (RPC `wh_apply_stock_delta`/`wh_transfer_stock` + عمود `orders.yurtici_exported_at`)، و**`warehouseService.js` صار يستخدم RPC ذرّياً** (receive/allocate/reserve/release/reverse — حلّ سباق #5/#15)، و**OrdersScreen يختم التصدير** (#16). build/lint/tests خضراء. (تأكيد نهائي مطلوب منك: عملية استلام/تخصيص فعلية — تجنّبتُ الكتابة على الإنتاج بالاختبار.)
- **⏳ يبقى عليك فقط:** (1) 🔴 **تدوير المفاتيح** — مُتحقَّق أن مفتاح `service_role` القديم **ما زال صالحاً** (لم يُدوَّر بعد) + توكنات `LOWES-SYRIA/TURKEY-2026` + `LOWES-META-SYNC-2026`. (2) نشر تصحيحات Edge للمصادقة الخادمية (#3 manage-employee · #10 ai-assistant · #20 track-yurtici · إزالة توكنات افتراضية #11/#19/#2/#12) بعد ضبط الأسرار. **مفصّل في `docs/SECURITY-REMEDIATION.md`؛ المرجع الكامل في `docs/audit-findings-full.txt`.**
- ⚠️ `exec_migration` RPC غير موجود وservice_role لا يطبّق DDL عبر REST، فأي DDL يلزمه SQL Editor بجلستك. **لم أُنفّذ commit/push** (خصوصاً تصحيحات Edge تتطلب تدوير الأسرار أولاً قبل أي نشر).
