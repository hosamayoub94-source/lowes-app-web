...
- **🔴 الدفع الجزئي:** قيد `orders_payment_status_check` كان يرفض `partial`. الإصلاح في `DB_RUN_THIS.sql` (جذر المشروع — المالك يشغّله؛ لا أملك صلاحية DDL إنتاج).
...
- **حدّ مهم:** تعديلات بنية القاعدة الإنتاجية محجوبة عن التنفيذ التلقائي بحاجز أمان لا يرفعه إذن المحادثة (الإضافي مرّ، لكن تعديل/حذف القيود يُحجب). الحل: المالك يشغّل `DB_RUN_THIS.sql` أو يضيف قاعدة صلاحية `Bash(node scripts/db-admin.mjs *)`.
