...
function MyTargetCard({ name }) {
  ...
  useEffect(() => {
    ...
    // هدف الشهر (يورّي $0 رغم تسليمات حقيقية).
    // المحاولة الأولى استخدمت order_status_history (to_status='delivered')
    // كمصدر تاريخ تسليم دقيق — لكن تحقّق حي كشف إنه غير موثوق: طلبات
    // status='delivered' فعلياً وupdated_at اليوم بلا أي سطر "→delivered"
    // بالتاريخ إطلاقاً (تسجيل الحالة بالجدول ناقص لبعض المسارات). صار
    // يعتمد على updated_at مباشرة — عمود موجود دائماً وموثوق (آخر تحديث
    // لطلب "delivered" هو عملياً لحظة تسليمه بأغلب الحالات)، باستعلام
