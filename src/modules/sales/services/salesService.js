...
let _mockReports = [
  ...
  { id: 'sr-2', report_date: '2026-05-18', total_orders: 62, total_sales_usd: 3100, total_ad_spend_usd: 850, roas: 3.65, status: 'submitted', notes: null, created_by: 'emp-1', created_at: '2026-05-18T20:00:00Z' },
];
let _mockChannelResults = [
  { id: 'cr-1', report_id: 'sr-1', channel_name: 'موقع إلكتروني', orders: 40, sales_usd: 2200 },
  { id: 'cr-2', report_id: 'sr-1', channel_name: 'إنستاغرام', orders: 30, sales_usd: 1400 },
  { id: 'cr-3', report_id: 'sr-1', channel_name: 'متجر', orders: 17, sales_usd: 720 },
];
let _mockAdResults = [
  { id: 'ar-1', report_id: 'sr-1', campaign_name: 'حملة مايو', platform: 'meta', ad_spend_usd: 600, orders: 45, revenue_usd: 2700, roas: 4.5 },
  { id: 'ar-2', report_id: 'sr-1', campaign_name: 'تيك توك ربيع', platform: 'tiktok', ad_spend_usd: 380, orders: 25, revenue_usd: 1500, roas: 3.95 },
];
let _mockChannels = [
...
export async function deleteReport(id) {
  if (USE_MOCK) { _mockReports = _mockReports.filter(r => r.id !== id); return; }
  const { supabase } = await import('@services/supabase');
...
export async function fetchChannelResults(reportId) {
  ...
  const { supabase } = await import('@services/supabase');
  const { data, error } = await supabase.from('daily_sales_channel_results').select('*, sales_channels(name)').eq('report_id', reportId);
  if (error) throw new Error(error.message);
  return data.map(r => ({ ...r, channel_name: r.channel_name ?? r.sales_channels?.name ?? '' }));
}

export async function fetchAdResults(reportId) {
  if (USE_MOCK) return _getMock().adResults.filter(a => a.report_id === reportId);
  const { supabase } = await import('@services/supabase');
  const { data, error } = await supabase.from('daily_sales_ad_results').select('*').eq('report_id', reportId);
  if (error) throw new Error(error.message);
  return data;
}
...
export async function createAdResult(data) {
  ...
  const { supabase } = await import('@services/supabase');
  const { data: row, error } = await supabase.from('daily_sales_ad_results').insert(data).select().single();
  if (error) throw new Error(error.message);
  return row;
}
...
export async function createChannelResult(data) {
  ...
  const { supabase } = await import('@services/supabase');
  const { data: row, error } = await supabase.from('daily_sales_channel_results').insert(data).select().single();
  if (error) throw new Error(error.message);
  return row;
}
