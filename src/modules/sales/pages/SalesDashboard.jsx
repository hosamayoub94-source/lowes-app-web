...
export function SalesDashboard() {
  ...
  const { createReport, selectReport, submitReport, approveReport, createAdResult, createChannelResult } = useSalesActions();
    ...
    for (const ad of ads) {
      try {
        await createAdResult({
          report_id: newReport.id, campaign_name: ad.campaign_name, platform: ad.platform,
          ad_spend_usd: Number(ad.ad_spend_usd)||0, orders: Number(ad.orders)||0, revenue_usd: Number(ad.revenue_usd)||0,
          roas: Number(ad.ad_spend_usd) > 0 ? Number((Number(ad.revenue_usd)/Number(ad.ad_spend_usd)).toFixed(2)) : 0,
        });
      } catch { /* continue */ }
    }
    for (const ch of chanRows) {
      try { await createChannelResult({ report_id: newReport.id, channel_name: ch.channel_name, orders: Number(ch.orders)||0, sales_usd: Number(ch.sales_usd)||0 }); } catch { /* continue */ }
    }
    setShowForm(false);
    ...
    <div className="min-h-screen bg-cream p-4 md:p-6" dir="rtl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          ...
          { label:'مبيعات آخر 7 أيام', value: usd(kpis.last7Sales),             icon:'💵', color:'text-green-600'   },
          { label:'إنفاق إعلاني',      value: usd(kpis.last7Spend),             icon:'📣', color:'text-orange-500'  },
        ...
        <div className="md:col-span-2">
          {!report ? (
            ...
            <div className="space-y-3">
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex gap-2">
                    {isAdmin && report.status === REPORT_STATUS.SUBMITTED && (
                      <button onClick={() => approveReport(report.id)} disabled={loading.action} className="px-3 py-1.5 rounded-lg bg-green-600 text-white text-xs font-semibold disabled:opacity-50">اعتماد</button>
                    )}
              ...
              {adResults.length > 0 && (
                <div className="bg-surface border border-border rounded-xl p-4">
                  <h3 className="text-sm font-semibold text-text mb-3">نتائج الحملات</h3>
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                      ...
                      <tbody>
                        {adResults.map(a => (
                          <tr key={a.id} className="border-b border-border last:border-0 hover:bg-cream/50">
                            <td className="py-2 px-2 text-right font-medium">{a.campaign_name}</td>
                      ...
                      <tfoot>
                        <tr className="border-t-2 border-border font-semibold text-muted">
                          <td className="py-2 px-2 text-xs" colSpan={2}>الإجمالي</td>
                          <td className="py-2 px-2 text-center text-orange-500">{usd(adResults.reduce((s,a) => s+Number(a.ad_spend_usd||0),0))}</td>
                          <td className="py-2 px-2 text-center">{adResults.reduce((s,a) => s+Number(a.orders||0),0)}</td>
                          <td className="py-2 px-2 text-center text-green-600">{usd(adResults.reduce((s,a) => s+Number(a.revenue_usd||0),0))}</td>
                          <td></td>
                        </tr>
                      </tfoot>
                    </table>
      ...
      <NewReportModal open={showForm} onClose={() => setShowForm(false)} onSave={handleCreate} loading={loading} campaigns={campaigns} channels={channels} />
    </div>
  );
}
