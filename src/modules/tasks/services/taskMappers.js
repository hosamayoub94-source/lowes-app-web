...
export function mapTask(row) {
  ...
  return {
    ...
    tagged_ids: Array.isArray(row.tagged_ids) ? row.tagged_ids : [],
...
function resolveAssignee(val) {
  const raw = typeof val === 'object' ? val?.id || null : val || null;
  return { assigned_to: raw, assignee_id: raw && UUID_RE.test(raw) ? raw : null };
}
...
export function toTaskInsert(input) {
  ...
  if ('assigned_to' in input) {
    Object.assign(row, resolveAssignee(input.assigned_to));
  }
...
export function toTaskUpdate(patch) {
  ...
  if ('assigned_to' in patch) {
    Object.assign(row, resolveAssignee(patch.assigned_to));
  }
  return row;
}
