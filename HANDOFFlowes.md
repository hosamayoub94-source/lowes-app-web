...
- **⏳ يحتاج تنفيذ من المالك (لا يمكن أتممه بصلاحياتي):** `supabase/migration_v12_lozy_kpi_coach_cron.sql` بـSQL Editor — يفعّل `pg_cron`/`pg_net` (صلاحية DB superuser) ويجدول تشغيل يومي 6ص UTC (~9ص تركيا/سوريا). **استبدل `SERVICE_ROLE_KEY_HERE` بمفتاح service_role الحقيقي** (Project Settings → API) قبل التنفيذ.
