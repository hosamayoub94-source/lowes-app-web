...
- الثلاث إصلاحات (صلاحيات CRM/Analytics، مودال CRM، تسمية الأدلة) اختُبرت حياً بالمتصفح (Chrome MCP) بحسابين حقيقيين (أدمن + موظفة عادية) — قبل وبعد الإصلاح، مو بس تحقّق كود.
- كل عملية حذف بيانات تحقّقت أولاً من الجداول المرتبطة (FK فعلي أو مرجع عملي) قبل التنفيذ.

#### ⏳ لم يُنفَّذ بعد (بانتظار الجلسة الجاية):
- تحقيق أعمق لماذا جدول `employee_kpis` فاضي بالكامل لكل الشهور (هل فيه job/cron مفروض يحسبه شهرياً وما بيشتغل؟)
- تحقيق تضارب بيانات الحضور بشاشة `/analytics` (بطاقة تعرض رقم حقيقي، رسم بياني تحتها "لا توجد بيانات")
