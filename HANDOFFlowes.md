...
### 🗓️ جلسة 23 يوليو 2026 — فلترة كشف الحركة + جرد البوليصة + بُغ تقرير الحملات + QR جديد ✅ build ✓ + مُتحقَّق حيّاً (/join)

#### ما أُنجز:

**📜 مشكلة 1+2 — فلترة كشف حركة المخزون (مخزن/تاريخ):**
- `MovementModal` (زر 📜 في `InventoryScreen.jsx`) كان يعرض آخر 60 حركة لكل الأسواق مخلوطة بلا فلترة — بالضبط ما يظهر بالصورة (سوريا/تركيا مختلطة).
- صار: فلتر سوق (🌍 الكل / 🇸🇾 سوريا / 🇹🇷 تركيا) + فلتر تاريخ (من/إلى) + ملخّص حيّ «🛒 خرج / ↩️ رجع / الصافي» للفترة المفلترة — هذا هو «الجرد الحقيقي» المطلوب (كم قطعة طلعت من مخزن معيّن بصنف وتاريخ محدَّدين). الحدّ رُفع من 60→500 حركة.
- الملف: `src/screens/InventoryScreen.jsx` (`MovementModal`)
- ملاحظة: تقرير «الجرد ناقص بالمستودع» (نقص/سالب) موجود مسبقاً بشاشة `/warehouses` (بطاقة كل مخزن تعرض عدد الأصناف السالبة + فلتر «⚠️ نواقص» + نافذة تفاصيل لكل مخزن) — لا حاجة لتعديل.

**🖨️ مشكلة 3 — جرد على البوليصة + حفظ يومي:**
- عند طباعة البوليصات تُضاف الآن صفحة/صفحتان أخيرتان «📋 جرد إغلاق اليوم» (لكل سوق ظاهر بالدفعة المطبوعة) — عمودان × جدول المنتج/الكمية.
- اللقطة تُحفَظ تلقائياً (upsert) بجدول جديد `inventory_daily_log` (تاريخ + سوق + منتج + كمية) لمراقبة الجرد يوماً بيوم لاحقاً. best-effort — لا تُفشِل الطباعة لو الجدول غير موجود بعد.
- الملف: `src/services/labelPrint.js` (`getInventorySnapshot` + `saveDailySnapshot` + `inventoryReportHTML`)
- **⏳ SQL يحتاجه المالك:** `supabase/migration_v11_inventory_daily_log.sql`

**📣 مشكلة 4 — بُغ حرِج: الموظفون لا يقدرون يفتحوا تقرير الحملات إطلاقاً:**
- السبب الجذري: مسار `/campaigns` كان محصوراً بـ`roles=[ADMIN,MANAGER,SALES_MANAGER,MEDIA_BUYER]` + صلاحية `MANAGE_CAMPAIGNS` — موظف عادي (`EMPLOYEE`) مُسنَد كعضو بحملة (`campaigns.members`) كان يُرمى فوراً لصفحة الهوم قبل ما يشوف الشاشة، رغم إن `CampaignsScreen.jsx` فيها أصلاً كود كامل لوضع «عرض الموظف» (يفلتر حملاته المُسنَدة، يخفي التكلفة، يعرض رابط «🧾 تقريري اليومي») — كان كوداً ميتاً غير قابل للوصول.
- الإصلاح: أُضيف `ROLES.EMPLOYEE` لمصفوفة الأدوار المسموحة بمسار `/campaigns` (`AppRoutes.jsx`) + لعنصر التنقّل `campaigns` بالسايدبار (`navigation.js`) — الآن الموظف المُسنَد يفتح الشاشة، يشوف إعلاناته، ويسجّل عبر «تقريري اليومي» (`/daily-report` — كان يعمل أصلاً لكن غير قابل للاكتشاف).
- تعدّد الموظفين على نفس الحملة (2-3 أشخاص) مدعوم أصلاً عبر `campaigns.members` (مصفوفة) + `report_ad_results` لكل (موظف×يوم×إعلان) — لا حاجة لتعديل.
- الملفات: `src/routes/AppRoutes.jsx` (route CAMPAIGNS) · `src/data/navigation.js` (nav item `campaigns`)

**📸 مشكلة 5 — تغيير QR البوليصة (صفحة انستا مُسترجَعة بحساب جديد):**
- الحساب الجديد: `@lowes_profesyonel` (`https://www.instagram.com/lowes_profesyonel/`) — مصدر واحد بـ`brand.js` (`COMPANY.instagramSkincare` / `instagramSkincareUrl`)، يغذّي: QR البوليصة (`labelPrint.js`)، بوليصة `OrdersScreen.jsx`، صفحة نجاح الانضمام (`JoinScreen.jsx` — كانت السطر الوحيد المكتوب يدوياً بلا مرجع مركزي، صُحِّح).
- **مُتحقَّق حيّاً:** فُتح `/join` بمتصفح فعلي، عُبّئ نموذج تجريبي، وظهر «📸 @lowes_profesyonel» بشاشة النجاح.
- ⚠️ **يحتاج تنظيف من المالك:** التحقّق الحيّ أدرج صفّاً تجريبياً بجدول `mlm_join_requests` (الاسم «اختبار تحقق QR»، الهاتف `0999999999`) — anon يكتب فقط لا يقرأ/يحذف بهذا الجدول (بالتصميم)، فلم أستطع تنظيفه. SQL للحذف: `DELETE FROM mlm_join_requests WHERE phone = '0999999999';`

#### مُتحقَّق:
- `npm run build` ✅ (مرّتين — قبل وبعد تعديل QR)
- `/join` حيّاً بمتصفح فعلي: النموذج يعمل، شاشة النجاح تعرض الحساب الجديد ✅

#### لم يُنشر بعد (untracked/uncommitted — بانتظار تأكيد المالك قبل commit/push):
- `src/screens/InventoryScreen.jsx` · `src/services/labelPrint.js` · `src/routes/AppRoutes.jsx` · `src/data/navigation.js` · `src/data/brand.js` · `src/screens/JoinScreen.jsx`
- **جديد:** `supabase/migration_v11_inventory_daily_log.sql`
...
2. **السايدبار:** مجموعة جديدة **«المالية»** (المصاريف والشحن + المالية العامة + قنوات المحاسبة) مفصولة عن «الإدارة» (الإدارة/المشاريع/الأدلة/الملفات/مساحة العمل). (`navigation.js` — group `finance` + `GROUP_ORDER`)
...
#### ما أُنجز (كود على main، build أخضر):
- **✅ إصلاح بُغ حرِج في MonthlyDeliveriesTab:** عند اختيار شهر ماضٍ كانت الطلبات المؤرشفة تختفي (الاستعلام الرئيسي يستثنيها). الإصلاح: `MonthlyDeliveriesTab` يجلب `archivedForPeriod` مستقلاً من Supabase لكل فترة مختارة ثم يدمجهما (بلا تكرار). الفلتر `delivered` صار `allForPeriod.filter(...)` بلا `o.archived === true`. يُنير الآن يونيو/مايو/أبريل حتى بعد أرشفتهم.
- **✅ سجل الموظفين الشهري (زر 📊 سجل الموظفين):** زر جديد داخل "تسليمات الشهر" يعرض جدول pivot: موظف × 6 أشهر الأخيرة (USD + عدد طلبات) + إجمالي تراكمي. يجلب البيانات مرة واحدة (lazy) ويتذكرها. يُظهر كل الأشهر (حية + مؤرشفة).
- **✅ أرشفة شهرية بالشهر لا 30 يوم:** `archiveOldDelivered` صار يأرشف كل الطلبات المسلّمة من الأشهر السابقة (قبل بداية الشهر الحالي)، لا «أقدم من 30 يوم» العشوائية. الزر يقول «🗄️ أرشفة هذا الشهر (N)» بدلاً من «أرشفة» مجردة.
- **⏳ هيا — تحتاج SQL فقط:**  `supabase/promote_hiya_turkey_sales_manager.sql` — شغّله من SQL Editor. يعطيها `role_type='sales_manager'` + `order_market='turkey'` → فتُمنح كل التابات: بوليصات 🖨️ + يورتيتشي + التجهيز + تسليمات الشهر + سجل الموظفين + الأرشيف + المحذوفة.

#### لم يُنشر بعد (يحتاج push):
```bash
git add src/screens/OrdersScreen.jsx supabase/promote_hiya_turkey_sales_manager.sql
git commit -m "feat(orders): monthly archive fix + employee records + hiya role SQL"
git push
```

### 🗓️ جلسة 3 يوليو 2026 (د) — أرشيف العملاء الشهري + صيد أعطال الإدراج ✅ منشور على main
- **✅ ميزة: أرشيف العملاء الشهري** (طلب المالك): زرّ «🗄️ الأرشيف الشهري» بشاشة العملاء → دلاء «تسليمات شهر كذا» (حسب شهر آخر طلب، الأحدث أولاً) بعدّاد لكل شهر + تحميل كسول عند الفتح، **يستثني الشهر الحالي غير المكتمل** + التواريخ المستقبلية الخاطئة، يحترم القسم و«عملائي». مُتحقَّق حيّاً (يونيو سوريا 361 · مايو 394 · أبريل 215، يوليو مستثنى). خدمة: `listCustomerMonths`+`monthRange`+وسائط `monthStart/monthEnd`.
- **✅ صيد أعطال إدراج (صنف عطل الإعلان — عمود legacy NOT NULL غير مكتوب):** فحص منهجي عبر OpenAPI (required cols) + إشارة «صفر صفوف رغم شاشة فعّالة». كُشف وأُصلح **`leave_requests.team`** (كانت كل طلبات الإجازة تفشل صامتاً، صفر صفوف إطلاقاً — الآن يكتب team من useAuth بـfallback). **سليمة (ليست أعطالاً):** mystery_shopper/daily_reports/tasks/quiz. طريقة الكشف الكاملة: `[[insert-schema-drift-bugs]]`.
...
- **✅ توحيد الحضور (بند خارطة):** جسر الرواتب كان يقرأ `attendance_records` (صفّان فقط) بينما الحيّ `attendance` (**4367 صفاً**، مفتاحه `employee_name` — `employee_id` فارغ للكل، التاريخ `YYYY/MM/DD`، حالات عربية `✅ حاضر`/`🚪 خروج`/`🌙 إجازة`). أعدت كتابة `attendanceLink.js` ليقرأ `attendance` بالاسم + `date like YYYY/MM/%`، يعدّ التواريخ الفريدة، يفصل الإجازة (يوم مُبرّر لا غياب)، مع مطابقة اسم مطبّعة احتياطية + fallback للقديم. **مُتحقَّق حيّاً عبر anon** (زينة يونيو: 26 حاضر/3 إجازة/0 غياب — والقديم كان يعطي الكل «26 غياب»). ملاحظة الحضور بكشف الرواتب صارت دقيقة (الخصم لا يزال يدويّاً بقرار المالك). مرجع: `[[payroll-engine-reorg-2026-07]]` (v4).
- **⏳ ما زال يحتاجك أنت فقط (SQL Editor / إدخال):** فهارس `orders_indexes_20260703.sql` (أداء) · ملء رواتب الـ12 موظفاً.

### 🗓️ جلسة 3 يوليو 2026 — تحقّق: إصلاح RLS (C1) مُطبَّق فعلاً على الإنتاج ✅
عند استئناف العمل على إصلاح RLS الحرِج (C1 من تدقيق 2/7)، أظهر الفحص الحيّ عبر REST أن **الإصلاح صار نافذاً على القاعدة** — anon (سياق جلسة الـPIN، `auth.uid()=NULL`) صار يرى نفس أعداد service_role بالضبط للجداول الثلاثة: `accounting_channels` **14/14** · `employee_requests` **1/1** · `activity_logs` **849/849** (كانت كلها 0 لدى anon في تدقيق 2/7). تحقّق إضافي: anon يقرأ صفوفاً فعلية (أسماء القنوات + صف السلف). **لا DDL متبقٍّ** — الشاشات الثلاث (منتقي القنوات + `/admin/channels` · الطلبات/السلف · لوحة التدقيق) تمتلئ لمستخدمي الـPIN، ومحرك الرواتب صار يرى السلف المعتمدة فيخصمها. غالباً طُبِّق `rls_manual_session_fix_20260702.sql` لحظة انقطاع الجلسة السابقة. مرجع: `[[rls-manual-session-lockouts]]`.
- **✅ نُفِّذ (بصفة المالك، منشور على main):** **C2** — بانر بارز في `/payroll`: «⚠️ N موظف بلا راتب أساسي مُدخل — سيُحتسب لهم صفر» + أسماؤهم قبل الاعتماد (مُتحقَّق حيّاً: 12/37 بدقّة). **M2** — تقوية مفاتيح ودجت `DashboardBuilder`؛ التحذير المتبقّي في `/analytics` dev-only حميد (recharts، يُجرَّد بالإنتاج).
- **⏳ يحتاج المالك (SQL Editor فقط):** `supabase/orders_indexes_20260703.sql` (M1 — فهارس أداء `orders`، سطراً سطراً بسبب `CONCURRENTLY`، آمن) · ملء رواتب الـ12 موظفاً من «الإعدادات → رواتب الموظفين».
- **ℹ️ M3:** الأربعة الفاشلة = ٣ ملغاة/محذوفة (يستثنيها البانر) + SL-2995 حقيقي (خطأ رابط Drive — يدوي لا إعادة محاولة). لا كود مطلوب.

### 🗓️ جلسة 2 يوليو 2026 (د) — تدقيق شامل كأدمن (44 مساراً + القاعدة) 🔎 قراءة فقط
جولة أدمن كاملة على كل شاشات التطبيق + فحص عميق للقاعدة الحيّة (صفر تعديل بيانات). التقرير الكامل: **`docs/audit-2026-07-02.md`**.
- **🔴 حرِج C1 — أقفال RLS تُفرِغ 3 شاشات لمستخدمي الـPIN (جلسة يدوية = anon، `auth.uid()=NULL`):** `accounting_channels` (anon يرى 0 / service 14 → منتقي القناة و/admin/channels فارغة)، `employee_requests` (0/1+ → شاشة السلف فارغة **ومحرك الرواتب لا يخصم السلف**)، `activity_logs` (0/844 → لوحة التدقيق فارغة). **الإصلاح جاهز:** `supabase/rls_manual_session_fix_20260702.sql` (يوسّع SELECT لـ`USING(true)`، آمن/idempotent) — **يطبّقه المالك من Supabase SQL Editor** (DDL لا يُطبّق عبر REST). مرجع: `[[rls-manual-session-lockouts]]`.
- **🔴 C2:** 12 من 37 موظفاً نشطاً بلا صفّ `employee_salary_settings` → «تشغيل الدورة» يعطيهم 0 صامتاً. المالك يملأ رواتبهم + نضيف تنبيهاً بارزاً.
- **🟠 متوسط:** M1 بطء استعلام `orders` (32,322 صف، 2–3.5s) → يحتاج فهارس `(market,created_at)`/`(status)`/`(handler_name)` · M2 تحذير React key في `/analytics` · M3 أربعة طلبات `sync_status=failed` عالقة.
- **🟢** صفر أخطاء console كاسرة عبر الـ44 مساراً · `/fulfillment` مسار غير مُعرّف (التجهيز زر داخلي) · خارطة تطوير كاملة (أتمتة/متانة بيانات/تجربة عالمية) في التقرير.

### 🗓️ جلسة 2 يوليو 2026 (ج) — دمج ونشر + تحديث دليل التطبيق ✅ منشور على الإنتاج
- **سبب «ما ظهرت البوليصة عالموبايل»:** المالك دمج PR #18 (البوليصات) لكن إصلاح الموبايل (4ec7584) كان لاحقاً على الفرع → **دُمج على main ودُفع** (aa1256b) وتحقّقتُ أن **الإنتاج الحي يقدّم الإصلاح فعلاً** (chunk OrdersScreen الحي فيه زر البوليصات + الالتفاف).
- **دليل التطبيق (app_guides، عبر REST بمفتاح service role — صار 21 دليلاً):** ① **الرواتب** أُعيدت كتابته بالكامل (الدورة بضغطة واحدة، الإعداد، العمولة فوق التارجت، الغياب/الرواجع يدوي، الاعتماد والدفع). ② جديد **«طباعة بوليصات الشحن»** (sales/45). ③ جديد **«التارجت والعمولات»** (admin/135، permission manage_payroll). ④ نصائح محدّثة: orders_syria/orders_turkey (إحالة للبوليصات) + attendance (الغياب لا يُخصم تلقائياً). التحديث upsert بـ`on_conflict=key` — آمن الإعادة.

### 🗓️ جلسة 2 يوليو 2026 (ب) — موبايل + عمولة فوق التارجت + الغياب/الرواجع يدوي ✅ (فرع `feat/shipping-labels`)
ثلاث نقاط من المالك:
1. **الموبايل ما يعرض كل الأزرار** (بوليصات وغيرها): هيدر `OrdersScreen` صار عمودياً على الموبايل والأزرار **تلتف** (`flex-wrap`) بدل ما تنقص خارج الشاشة.
2. **العمولة صارت «فوق التارجت» فقط** (قرار المالك يلغي النسبة الثابتة من 1/7): محرك الرواتب يقرأ **نفس `commission_rules`** التي يعدّلها الأدمن من شاشة الطلبات (⚙️ إعدادات العمولات) — تركيا: فوق `monthly_target_try` (65k) → `above_target_pct` (5%) على الفائض + شرائح مسبق الدفع؛ سوريا: فوق `monthly_target_usd` بالدولار المحوَّل. `commissionFromIndex()` في `payrollEngine.js` (مطابق لـ`commissionBreakdown` بمحفظة البائع). تفصيل الحساب يُكتب بملاحظات كل موظف. fallback للنسبة الثابتة لو فشل جلب القواعد.
3. **الغياب: لا خصم تلقائي** («سجّلهم كلهم حضور») — المحرك يكتب غياب=0 دائماً ويعرض الحضور المسجّل كمرجع بالملاحظات؛ الأدمن يحدد الغياب يدوياً من ✏️ (الحقول كانت قابلة للتحرير أصلاً). 4. **الرواجع/غير المستلَم:** يدوي بالكامل — الأدمن يعدّل من «خصومات أخرى»/«العمولة» بالمودال (الرواجع أصلاً لا تُحسب مبيعات لأن العمولة على delivered/settled فقط).
**تحقّق:** build ✅ + اختبار المنطق حيّاً بالمتصفح: تركيا 80k₺ (+20k مسبق) → ₺2,000 = $60.61 · 50k → 0 · سوريا $1,500 مقابل تارجت $1,000 @10% → $50. مرجع: `[[payroll-engine-reorg-2026-07]]` (v3).

### 🗓️ جلسة 2 يوليو 2026 — طباعة بوليصات الشحن بهوية البراند (سوريا + موتور تركيا) ✅ مبني + مُعايَن بصرياً (فرع `feat/shipping-labels`)
طلب المالك: طباعة بوليصات للطلبات (سوريا: «في التجهيز» · تركيا: «تحضير الموتور» فقط — يورتيتشي لها بوليصتها) بشكل احترافي مطابق لمثال PDF أرسله (`LOWES_SY_2026-07-02…pdf`)، مع QR لصفحة الانستغرام الجديدة ورسائل تليق بالبراند وإيميل الشركة.
- **الشكل (مطابق للمثال + هوية البراند):** A4 = **8 بوليصات (2×4)** — رأس LOWE'S Professional (Playfair متباعد + خط ذهبي متدرّج) · عدّاد n/N + تاريخ/وقت · اسم العميل (El Messiri) · 📞/💬 · 📍 العنوان (مدينة–منطقة–حي–تفصيلي، بلا تكرار) · 📦 رقم الطلب بصندوق ذهبي · 🚚 شركة الشحن · **🎀 رسالة شخصية باسم العميل الأول 🤍** (12 رسالة راقية تُختار بثبات حسب رقم الطلب — إعادة الطباعة = نفس الرسالة) · سطر الدفع (`payment_status`: مدفوع → «✅ مدفوع مسبقاً — أجرة الشحن على المرسل» وإلا «💳 التحصيل على العميل» + 💰 المتبقي فعلياً مع مراعاة الجزئي) · تذييل: **QR** (SVG مضمّن inline — يعمل بلا شبكة) لـ`instagram.com/lowes.skincare` + 📞 خدمة العملاء لكل سوق + 📧 `info@lowesprofesyonel.com` + السلوغان.
- **الملفات:** `src/services/labelPrint.js` (`buildLabelsHTML` مفصولة للاختبار + `openLabelsPrint` + `labelEligible`) · `src/components/feature/LabelPrintModal.jsx` (اختيار الطلبات، مجموعات سوريا/تركيا، تحديد/إلغاء الكل، عدّ الأوراق) · `src/assets/ig-qr.svg` (QR مولّد مسبقاً عبر `npx qrcode`، يُستورد `?raw`) · زر **«🖨️ بوليصات (N)»** بهيدر `OrdersScreen` (isManager/isFulfillment) · `brand.js`: أُضيف `COMPANY.instagramSkincare(Url)` + `COMPANY.customerService` (سوريا `0931471996` / تركيا الواتساب).
- **التحقّق:** `npm run build` ✅ · **معاينة بصرية حيّة** (حقن `buildLabelsHTML` بعينات في iframe عبر dev server): الورقة كاملة 8 بوليصات، صفر overflow بعد ضبط المقاسات، QR والرسائل والعملات (ل.س/$/₺) والمدفوع مسبقاً كلها صحيحة. ⚠️ لم يُختبَر النقر الفعلي بجلسة مسجّلة (لا PIN متاح) — المالك يجرّب الزر بعد النشر من `/orders`.
- **ملاحظة بنية:** العمل على فرع جديد `feat/shipping-labels` مبني على `origin/main` (فرع `feat/payroll-engine` كان مدموجاً وريموته محذوف؛ ملفات untracked متصادمة مع main نُسخت احتياطياً للـscratchpad). مرجع ذاكرة: `[[shipping-labels-print-2026-07]]`.

### 🗓️ جلسة 1 يوليو 2026 — إعادة تنظيم الرواتب: «محرّك رواتب» بضغطة واحدة ✅ منجز + مطبّق على الإنتاج + مدموج على main
طلب المالك: «حساب الرواتب — أيام الدوام، كشف حركة المبيعات للموظفين — بلا تدخّل فردي لكل موظف، ونظّمه على الطرق العالمية». تدقيق محاسبي كامل ثم بناء محرّك موحّد ونشره.
- **التدقيق كشف الانحراف:** مصدر الراتب الحيّ = **`profiles`** (`base_salary_usd`, `housing_allowance_usd`, `transport_allowance_usd`, `commission_pct`) عبر `/admin/users` — لا `employee_salary_settings` (غير مستخدم). المبيعات تُنسب عبر **`orders.handler_name`** (نص). تعارض مخططات `payroll_entries` (نسخة `_usd` من `migration_v5` هي الحيّة، لا `0005`). `employee_kpis`/`commissions` مُشار إليهما بالكود بلا migration (نظام KPI منفصل في PerformanceScreen).
- **قرارات المالك:** كل موظف **بعملته** (TRY/SYP/USD) · عمولة **نسبة % ثابتة** (`commission_pct`) · أتمتة الأربعة (أساسي+بدلات · غياب · عمولة · سلف).
- **المبني:**
  - `supabase/migrations/20260701_payroll_engine.sql` — **إضافي آمن فقط** (`ADD COLUMN IF NOT EXISTS`): `profiles.salary_currency`(+`seller_alias`)+GRANT، وأعمدة تفصيل على `payroll_entries` (currency, allowances_usd, commission_usd, commission_pct, sales_total_usd, sales_orders_count, absence_deduction_usd, source, computed_at).
  - `src/modules/payroll/services/payrollEngine.js` — يجمّع مبيعات الشهر المحصّلة (`status IN delivered,settled`) من `orders` حسب اسم البائع المطبّع، خصم الغياب من `attendance_records`، السلف من `employee_requests` (repay_month)، ويحسب صافي كل موظف بعملته. `runPayrollForMonth` يشغّل الجميع دفعة واحدة (idempotent عبر `upsert onConflict run_id,employee_id`).
  - الواجهة (`PayrollDashboard.jsx`): زر واحد **«⚡ تشغيل الدورة»** بدل الملء/العمولة اليدويين + عمود «العمولة» + مودال **«كشف حركة المبيعات»** لكل موظف (يعرض طلباته) + إجماليات لكل عملة. `EntryRow`/`printPayslip`/`calcNetSalary` صاروا بالعملة الأصلية ويشملون البدلات+العمولة (متوافق خلفياً مع الإدخالات القديمة).
- **التحقّق (فعلي):** `npm run build` ✅ · اختبار الحساب ✅ (net TRY = ₺24500، legacy net = 550) · **الـmigration مطبّق على الإنتاج عبر SQL Editor** ومتحقَّق مستقلاً بـREST (الأعمدة + GRANT anon تعمل → الدخول سليم) · **اختبار كتابة e2e** على الإنتاج (upsert idempotent بالأعمدة الجديدة + عملة TRY تُحفظ، وبيانات الاختبار حُذفت).
- **بعد النشر (إعداد المالك):** عيّن عملة كل موظف + `commission_pct` من `/admin/users`، وتأكّد أن `handler_name` في الطلبات يطابق اسم الموظف (أو عيّن `seller_alias`). ثم `/payroll` → «دورة جديدة» → «⚡ تشغيل الدورة». التصميم الكامل: `docs/payroll-engine-blueprint.md`. مرجع ذاكرة: `[[payroll-engine-reorg-2026-07]]`.
- **متبقٍّ (مراحل لاحقة):** توحيد جداول المبيعات/الحضور المكرّرة · ربط `orders.seller_id` (UUID) بدل الاسم النصي · معاينة حيّة كاملة داخل التطبيق بحساب أدمن.

### 🗓️ جلسة 25 يونيو 2026 (ج) — أتمتة العنوان التركي + حلّ المزامنة الجذري الدائم ✅ (مُختبَر حيّاً)
طلب المالك: (١) أتمتة العنوان التركي مثل turktelekom (Mahalle أحياناً «ما بياخد») · (٢) حلّ جذري دائم لفشل المزامنة بدل إعادة relax كل فترة.
- **🟢 المزامنة — الحلّ الجذري (منشور حيّ + مُختبَر):** `doPost` في مشروع تركيا الحيّ `1ub2zm…` صار **يداوي نفسه** — `_writeRowRelaxed(sh,row,w,vals)` يُرخّي تحقّق «رفض» المنسدلة على الصف **قبل كل كتابة** (يبقي القوائم، يمنع `bad_response` نهائياً). + تريغر `onChange` تلقائي (`onTurkeyValidationChange`/`createRelaxTrigger`، مُثبَّت) يُرخّي لو رجّع الفريق «رفض». **نُشر كالإصدار 39** على نفس `/exec` (TURKEY_SHEET_SYNC_URL سليم). **تحقّق:** اختبار ذاتي PASS (وضعت «رفض» فعلي + كتبت قيمة مرفوضة → نجحت + relaxedNow=true) · end-to-end: **6S68** (Haneen Mohamad، كان يفشل على J35) صار `synced` عبر الـedge fn. **ما عاد يلزم تشغيل relax يدوي.** الملفّان المرجعيّان بالريبو محدَّثان (تركيا + سوريا `_relaxRow`). ⚠️ **سوريا (`1Dpnbvh5…`) لم يُنشَر بعد** (الملف جاهز — متابعة اختيارية).
- **🟢 العنوان (مبني + مُختبَر حيّاً، يحتاج نشر Vercel):** `fetchMahalles` يجلب الأحياء من **نفس بيانات الشوارع المدمجة** `/tr-streets/{id}.json` (مفاتيحها = أسماء الأحياء) بدل API خارجي flaky → يحلّ «ما بياخد الـ Mahalle». **+ إصلاح جذري لخلل تطبيع قديم** (كان يكسر **كل إسطنبول** للأحياء والشوارع معاً): الفهرس فيه `kadiköy`/`denizli̇` بينما البحث `kadıköy` — `nkey` صار يجرّد U+0307 ويحوّل ı→i، و`loadStreetIndex` يعيد ترقيم الفهرس. **تحقّق حيّ بالفورم:** İstanbul→Kadıköy→قائمة أحياء (CADDEBOSTAN…)→شوارع (BAĞDAT…)→بُني «CADDEBOSTAN Mah. BAĞDAT Sok. No:12 D:5». (Kadıköy كانت ترجع صفر، الآن ٢١؛ Esenyurt ٤٣/١٧١.) + `parseTurkishAddress` يعيد ملء الأجزاء عند التعديل · بناء/شقة `ComboBox` بذاكرة · تعبئة عنوان آخر طلب للعميل المتكرّر. + `failedSync`/`listFailedSync` يستثنيان الحالات النهائية (لا يبقى طلب منتهٍ كفشل بالبانر).
- **الملفات:** `src/services/turkeyApi.js` · `src/data/cities.js` · `src/screens/OrdersScreen.jsx` · `src/services/orderSyncService.js` · `google-apps-script/{turkey,lowes}-sales-sync.gs`. الخطة: `~/.claude/plans/toasty-mixing-moler.md`. مرجع ذاكرة: `[[strong-orders-sync-fixes-2026-06]]`.

### 🗓️ جلسة 25 يونيو 2026 (ب) — إصلاح ٥ أعطال «طلبات Strong/تركيا» (تشخيص جذري + إصلاح حيّ متحقَّق) ✅
شكوى المالك: (١) منتجات سترونغ ماعم تبين بالتطبيق · (٢) طلبات محذوفة موجودة بالجدول · (٣) ماعم يتبع الطلبات · (٤) تعديل الطلب بينزل مرتين بالجدول · (٥) يقول «تحدّث» وما بيتحدّث. تشخيص بالأدلة الحيّة (REST + قراءة الجداول عبر gviz بجلسة المالك):
- **(١) السبب: المنتجات السبعة فئة `Strong` كلها `is_active=false`** (super viga ×4، viagra/cialis/Female.Viagra)، وفورم الطلب يعرض النشط فقط (`OrdersScreen.jsx:1813` `.eq('is_active',true)`). **الإصلاح:** `UPDATE products SET is_active=true WHERE category='Strong'` عبر SQL Editor (دور postgres) → ٧ صفوف، متحقَّق.
- **(٤)(٥) السبب الجذري للتكرار/«متزامن كذباً»: أكواد التطبيق القديمة العشوائية/المتضاربة** (6L###/6S###، قبل الهجرة) كانت تكسر مطابقة الصف في doPost فيُلحَق صف ثانٍ. **مُصلَح أماماً بهجرة الأكواد التسلسلية `f8be10a`** (TL-/TS-/SL-) — **صفر تكرار بالأكواد الجديدة**. نُظّفت **٥ بقايا** في `LOWES_TR` (6L607/611/612/619/641، أُبقيت نسخة «تم التسليم») عبر دالة دفاعية `dedupLowesTR_20260625` (ملف `DedupCleanup.gs` بمشروع تركيا الحيّ 1ub2zm). تحقّق: LOWES_TR صار صفر تكرار (١٣١→١٢٦) · STRONG_TR صفر · سوريا صفر.
- **(٢) السبب: ١٠ طلبات سترونغ قديمة فشلت مزامنتها الأمامية** بخطأ `bad_response` على **عمود J «صاحب الطلب»** (تحقّق منسدلة «رفض») لأسماء بائعين غير مطابقة حرفياً (Sarah Ibrahim≠Sara Ibrahem، Haneen Mohamad≠Haneen…) — **كلها قبل relax ٢٢ يونيو**. ٦ نشطة كانت **مفقودة من الجدول** (6S77/81/82/83/84/85) → أُعيدت مزامنتها عبر دالة `sync-order-to-sheet` فنزلت صفوف ٤٣–٤٨ **بأسماء بائعيها بلا رفض** (التحقّق صار مُرخّى للصفوف الجديدة)، صارت `synced`. الملغيان المحذوفان 6S76/6S678 تُركا (محذوفان أصلاً).
- **وقاية:** شُغّلت `relaxAllStatusValidation` على مشروع تركيا الحيّ → **`STRONG_TR: relaxed 0 cells | LOWES_TR: relaxed 0 cells`** = **لا يوجد حالياً أي تحقّق «رفض»** (relax ٢٢ يونيو ممسوك). ⚠️ يتكرّر لو الفريق رجّع منسدلة «رفض» → أعِد تشغيل الدالة (نفس مشروع 1ub2zm، Run). ترقية اختيارية: تريغر onChange يُرخّي تلقائياً.
- **(٣) التتبّع العكسي شغّال فعلياً** (شوهدت تحديثات حيّة `source=sheet` و`source=yurtici` اليوم بانتظام) — لم يكن معطّلاً؛ المشكلة كانت الطلبات الـ٦ المفقودة (صارت تتبع الآن بعد رجوعها للجدول).
- **تقنية مفيدة (مُثبتة هذه الجلسة):** قراءة أي تاب من جدول جوجل عبر `fetch('…/gviz/tq?tqx=out:csv&sheet=NAME', {credentials:'include'})` من نطاق docs.google.com بجلسة المالك (يتجاوز حجب gviz العام). · Extensions→Apps Script من جدول تركيا يفتح **المشروع الحيّ 1ub2zm** مباشرةً (فيه Dashboard.gs/RelaxStatusValidation.gs/YurticiPublicTracker.gs). مرجع ذاكرة: `[[strong-orders-sync-fixes-2026-06]]`.

### 🗓️ جلسة 25 يونيو 2026 — تحقّق النشر: كل شي على main فعلاً + الموقع الحيّ سليم ✅
تبيّن إنّ كل الشغل اللي إدخالات سابقة تقول عنه «لم يُنشر بعد» هو فعلاً **مدفوع على `origin/main` ومنشور** (الهاندوف كان قديم). الفرع `claude/pedantic-hamilton-cd34d7` = `origin/main` بالضبط (٠ commits معلّقة، شجرة نظيفة). التأكيدات: م1 كشف حركة الحساب (`fb33e44`) · م2 مشغّل الأوامر+المفضّلة (`3c2f0a9`) · أكواد الطلبات التسلسلية+دفعة الفورم (`f8be10a`) · إصلاح العملاء/الأرشيف+شركات الشحن القابلة للإدارة (`6a6766f`). هجرتا `0007`/`0008` **مطبّقتان حيّاً** (٢٤ يونيو، راجع الذاكرة).
**تحقّق حيّ آمن (قراءة فقط) على `lowes-app-web.vercel.app`:** الرئيسية تفتح كاملة (حضور/مبيعات/ليدربورد/أسعار صرف) · **صفر أخطاء console** وتسلسل الإقلاع نظيف (Queue/Automation/Analytics/Environment OK/Events) · **Ctrl+K يفتح مشغّل الأوامر** فعلاً (م2 شغّالة). لم تُنشأ طلبات تجريبية (تجنّباً للكتابة على الإنتاج/الشيت).
**✅ تحقّق تسلسل الأكواد (٢٥/٦، قراءة-فقط على القاعدة الحيّة عبر REST):** طلبات إنتاج حقيقية أخذت الأكواد بالضبط من البذور وتتسلسل بلا فجوات ولا عشوائية — **TL-28580→28587** · **TS-11051→11053** · **SL-2890→2897** · **SS- = صفر** (تأكيد دمج سوريا على SL-). الميزة شغّالة فعلياً بالإنتاج، فلا حاجة لطلب اختبار. (العدّادات `order_code_counters` محميّة بـRLS — anon ما بيقراها؛ التحقّق تمّ من أكواد جدول `orders`.)
**يتبقّى اختيارياً فقط:** **ربط ميتا التلقائي مؤجّل بقرار المالك (٢٥/٦)** — اختياري، ROAS اليدوي شغّال؛ كل التفاصيل والمعرّفات بذاكرة `[[meta-ads-sync-setup]]`.

### 🗓️ جلسة 22 يونيو 2026 (ج) — دفعة تحسينات فورم الطلب (تدقيق خبير + مراجعة عدائية) ✅ مبني ومُراجَع
بعد تدقيق خبير شامل لتدفّق إنشاء الطلب، نُفّذت ٨ إصلاحات في `OrdersScreen.jsx` (+`orderSyncService.js`) بمنهجية: workflow تصميم متوازٍ (٩ وكلاء) → تنفيذ متسلسل → workflow مراجعة عدائية ٦ أبعاد (٥ clean + بُعد التاريخ صُحّح):
1. **تحقّق مالي**: تحذير عند مبلغ صفر/فارغ، مدفوع>الإجمالي، اتّساق الدفع الجزئي؛ وتعبئة المدفوع=الإجمالي عند «مدفوع كامل».
2. **نافذة تأكيد واحدة** مدمجة بدل اثنتين متتاليتين (التنبيهات + التكرار).
3. **كشف التكرار/العميل المكرر يشمل رقم الواتساب** (`phone_1 || wa_number || phone_2`) — أساسي لعملاء سوريا (واتساب فقط).
4. **`order_date` بالتوقيت المحلي**: يُخزَّن wall-clock+`Z` (لا `toISOString`/UTC)، والافتراضي `localNow()`، والعرض `fmtOrderDay()` (يقطع التاريخ ويعرضه UTC) — فيطابق يومُ الطلب يومَ الموظف وإسناد العمولة/المحاسبة. ⚠️ **قاعدة:** لا تُحوّل `order_date` بـ`new Date().toISOString()` عند الحفظ.
5. **دمج المنتجات المكرّرة** بجمع الكمية. 6. **كتابة الكمية يدوياً** (مع +/−). 7. مطابقة كود «توصيل الموتور 🏍️» للقائمة. 8. **تسمية المنتجات الفاشلة** بالمزامنة (تمرير `droppedItems`؛ يتطلب لاحقاً أن يرجّعها Apps Script — يرجع لرسالة العدد تلقائياً).
`npm run build` ✅ (1246 module). تبقّى High بانتظار قرار المالك: لا شي حرِج (التدقيق سجّل بنوداً Medium/Low أخرى). **بعدُ:** تجربة طلب حقيقي لكل فريق بعد النشر.

### 🗓️ جلسة 22 يونيو 2026 (ب) — توليد أكواد الطلبات تسلسلي لكل فريق ✅ مُطبَّق على القاعدة + كود التطبيق (لم يُنشر بعد)
المشكلة: أكواد الطلبات «عشوائية» ومتلخبطة بين الفِرق (سوريا/تركيا). **السبب:** `nextOrderId`/`ensureUniqueOrderId` بـ`OrdersScreen.jsx` تحسب الرقم من قائمة محمّلة جزئياً (`.limit(500)` بدون محذوف/مؤرشف) ثم تمشي رقم‑رقم بالقاعدة، وبعد **200 محاولة** تُرجِع كوداً بلاحقة وقت (`…-7432` = العشوائي) + غير ذرّي (سباق) + سترونغ ولويز تركيا يتشاركوا سلسلة `S`.
**الواقع (32,071 طلب):** الأغلب أكواد مستوردة `ARC-T-` (تركيا، max 28579 لويز / 11050 سترونغ) و`ARC-` (سوريا، 2889 لويز / 2239 سترونغ)، والتطبيق يولّد `ML/MS/MSA` قصيرة غير متّسقة.
**الحل (مُطبَّق حيّاً على القاعدة عبر SQL Editor):** [`supabase/migrations/20260622_order_code_sequential.sql`] — جدول `order_code_counters` + تريغر `BEFORE INSERT assign_order_code` (SECURITY DEFINER، ذرّي عبر `UPDATE…RETURNING`). عدّاد تسلسلي لكل فريق (market+brand) ببادئة نظيفة مزروعة من أعلى رقم: **TL-** تركيا-لويز(28579) · **TS-** تركيا-سترونغ(11050) · **SL-** سوريا-لويز(2889) · **SS-** سوريا-سترونغ(2239). البادئات الجديدة غير موجودة بأي كود حالي → **صفر تضارب** مع `UNIQUE(order_id)`. أول كود: TL-28580 / TS-11051 / SL-2890 / SS-2240.
**تَحقّق حيّ:** seeds صحيحة؛ التريغر يولّد تسلسلياً (TL-28580 ثم TL-28581) لكل الفِرق؛ الأكواد المزوّدة تمرّ بلا تغيير (الاستيراد/الشيت آمن)؛ كل الاختبارات بـ `ROLLBACK` فما استهلكت العدّادات.
**كود التطبيق (`OrdersScreen.jsx`):** حُذف `nextOrderId`/`ensureUniqueOrderId` والتعبئة المسبقة؛ الإنشاء يرسل `order_id` فارغاً فيملؤه التريغر؛ `.select('id, order_id')` يقرأ الكود المولّد ويظهره بالتوست («· كود TL-28580»)؛ حقل «رقم الطلب» صار للقراءة فقط (تلقائي للجديد، يعرض الكود الموجود للتعديل). `npm run build` ✅.
**تصحيح سوريا (سوريا ما فيها سترونغ):** التريغر يجمع كل طلبات سوريا بسلسلة واحدة `SL-` (`CASE WHEN market='syria' THEN 'syria:lowes'`)، وحُذف عدّاد `syria:strong` (صار 3 فِرق: TL-/TS-/SL-). تَحقّق: طلب `syria+strong` صار ياخد `SL-2890` مش `SS-`. **سبب الجذر:** مبدّل البراند ظاهر لتركيا فقط، لكن `form.brand` ما كان يُصفّر عند تبديل السوق → يتسرّب `brand='strong'` على طلبات سوريا (109 صف قديم). أُصلح بالعميل: `handleMarketChange` + فرع prefill + ثابت وقت الحفظ يفرضون `brand='lowes'` لسوريا، مع تصفير حقول العنوان عند تبديل السوق. (تنظيف اختياري للـ109 صف: `UPDATE orders SET brand='lowes' WHERE market='syria' AND brand='strong'`).
**تدقيق تجربة المستخدم (جلسة 22/6):** أُجري تدقيق خبير على كامل تدفّق إنشاء الطلب — أُصلحت الأعطال الحرجة (تسريب براند سوريا، حالة تركيا غير صالحة، تصفير الحقول). تبقّى أعطال High/Medium بانتظار قرار المالك (تحقّق مالي، كشف التكرار/العميل المكرر على رقم الواتساب أيضاً، نافذتا confirm، إدخال الكمية يدوياً…). 
**بعدُ:** نشر `lowes-app-web` (Vercel) ثم تأكيد إنشاء طلب حقيقي ياخد الكود التالي. مرجع ذاكرة: `[[order-code-sequential-generation]]`.

### 🗓️ جلسة 22 يونيو 2026 — إصلاح مزامنة «طلبات تركيا سترونغ» (bad_response على J35) ✅ مُنجَز ومُتحقَّق حيّاً
عطل: كرت طلب سترونغ يُظهر «لم ينزل على الجدول (bad_response)» + خطأ تحقّق Google على الخلية **J35**. **السبب الجذري:** عمود **J `صاحب الطلب` (البائع)** في تاب **`STRONG_TR`** عليه قائمة منسدلة بوضع **«رفض الإدخال»**؛ `setValue` بـApps Script يرمي استثناءً فيفشل `doPost` → `bad_response` (نفس صنف عطلَي العمود I/T السابقَين). الفريق أضاف منسدلة البائع **بعد** تشغيل relax بـ10 يونيو (الذي سجّل `STRONG_TR=0` لأنها لم تكن موجودة بعد). **الإصلاح (قاعدة المالك الدائمة: أي منسدلة = «تحذير» لا «رفض»):** شُغّلت **`relaxAllStatusValidation`** على المشروع الحيّ `1ub2zm…` (ملف `RelaxStatusValidation.gs`، عبر Chrome) → السجل: `STRONG_TR: relaxed 383 cells | LOWES_TR: relaxed 354 cells`. **تحقّق حيّ على الخلية الفاشلة:** قاعدة `STRONG_TR!J24:J386` صار خيار «إذا كانت البيانات غير صالحة» = **«إظهار تحذير»** مع بقاء قائمة الأسماء كاملة → `setValue` لم يعد يرمي، والطلب يُزامَن تلقائياً (auto-retry). **يتكرّر** كلّما أضاف/أعاد الفريق إنشاء منسدلة «رفض» على `STRONG_TR`/`LOWES_TR` (التابان الوحيدان اللذان يكتب إليهما التطبيق عبر `SHEET_FOR`) → الحلّ: أعِد تشغيل الدالة (آمنة وidempotent، ~دقيقتان). ترقية اختيارية لاحقاً: تريغر `onChange` يُرخّي تلقائياً. مرجع ذاكرة: `[[turkey-orders-sheet-sync-reject-validation]]`.

### 🗓️ جلسة 17 يونيو 2026 — المرحلة 2: تنظيم ذكي + بحث سريع ✅ مُنجَز ومُتحقَّق حيّاً (worktree — لم يُنشر بعد)
طلب المالك: «صار فيه أدوات كثيرة — رتّب التطبيق وسهّل الوصول، وسجّل غير المستخدم للمراجعة». القرار: تنظيم ذكي + بحث سريع.
- **مشغّل أوامر (Command Palette)** `components/ui/CommandPalette.jsx` — يُفتح بـ**Ctrl/Cmd+K** (مستمع عام في `MainLayout`) أو **زر بحث في الهيدر** (`Header.jsx`، prop `onOpenSearch`). يبحث في `navItemsForRole(role, permissions)` فقط (يحترم الدور/الصلاحية) ببحث عربي مطبّع، تنقّل بالكيبورد (↑↓/Enter/Esc)، ونجمة تثبيت لكل أداة. عند فراغ البحث يعرض المفضّلة أولاً.
- **أدوات مفضّلة** `hooks/useFavorites.js` (localStorage لكل مستخدم `lowes:favorites:<id>` + مزامنة فورية) — ☆/⭐ في **المشغّل** و**الشريط الجانبي** (`Sidebar.jsx`، تظهر بالـhover أو دائماً للمثبّتة) + شبكة **«أدواتي المفضّلة»** على الرئيسية (`components/feature/FavoritesQuickAccess.jsx`، تظهر فقط عند وجود مفضّلات).
- **مراجعة الأدوات (بلا حذف):** `docs/unused-tools-review.md` — مرشّحون: تداخل `/requests`↔`/leave`، تعدّد لوحات التحليل، `/workspace`، شاشات تجريبية (lozy/face-enroll/quiz)، stubs (`queueHandlers.js`, `guides.js`). + خارطة `docs/roadmap.md`.
- **التحقّق:** `npm run build` أخضر + ملفاتي lint-نظيفة (الأخطاء المتبقّية قديمة: `Lowe's` في Sidebar + استيرادات HomeScreen). **معاينة حيّة:** Ctrl+K يفتح المشغّل · بحث «تقرير» → 3 نتائج · تثبيت 3 أدوات → `["workspace","accounting","orders-syria"]` بالتخزين + ظهورها بشبكة الرئيسية وبنجمة الشريط الجانبي. بلا أخطاء console (عدا تحذير Supabase env للـworktree).
- **متبقٍّ:** م3 (عقود/وثائق الموظفين) **مواصفة جاهزة** `docs/superpowers/specs/employee-contracts-design.md` — تُنفَّذ لاحقاً بطلب المالك · م4 خارطة الأدوات الناقصة (كشف حساب عميل/مورّد، تذكيرات، تدفّق نقدي…).

### 🗓️ جلسة 17 يونيو 2026 — المرحلة 1: كشف حركة الحساب (محافظ + قنوات) ✅ مُنجَز ومُتحقَّق حيّاً (على فرع worktree — لم يُنشر بعد)
طلب المالك (خطة من 4 مراحل): «عند النقر على كل محفظة يظهر كشف حركة الحساب» (مثال: حركة الشام كاش فقط، أو القدموس). القرار: المحافظ + القنوات معاً.
- **مكوّن موحّد `AccountStatement.jsx`** (Modal بإعادة استخدام `components/ui/Modal`) يُفتح عند النقر:
  - **المحافظ:** `TreasuryPanel` — كل بطاقة محفظة صارت `<button>` → كشف حركتها (عملة واحدة، اتجاه عبر `walletDelta` فيشمل التحويلات والمحافظ القديمة `sham_cash`).
  - **القنوات:** `ChannelPnL` — كل صفّ قناة صار قابلاً للنقر → كشف حركتها (متعدّد العملات، دخل=وارد/مصروف=صادر، التحويلات مُستثناة). يدعم قناة مُدارة (`channel_id`) أو مصدراً بلا قناة (اسم `category`). + prop `statementEntries` يمرّر **التاريخ الكامل** للكشف (الشاشتان تمرّران كل القيود، بينما جدول PnL يبقى شهرياً).
  - الكشف: جدول التاريخ/النوع/الوصف · وارد · صادر · **رصيد جارٍ لكل عملة** + فلترة (من/إلى تاريخ + بحث) + إجماليات + **تصدير Excel**.
- **منطق نقيّ** `accountStatement.logic.js` (`walletStatementRows`/`channelStatementRows`/`buildStatement`) — الترتيب زمنيّ صاعد لرصيد جارٍ صحيح.
- **التحقّق:** `test-account-statement.mjs` **20/20** + الاختبارات السابقة سليمة (channel-pnl 10/10 · source-breakdown 16/16 · accounting-books 9/9) + `npm run build` أخضر + lint نظيف. **معاينة حيّة** (حقن جلسة admin في وضع mock بالـworktree): نقر «بنك USD» → كشف بـ3 قيود، رصيد جارٍ −$1,200→−$2,000→$1,200، إجماليات (وارد $3,200/صادر $2,000/صافي $1,200) **مطابقة لبطاقة المحفظة** · نقر «إعلانات ميتا» (قناة بلا channel_id) → كشف صحيح. بلا أخطاء console (عدا تحذير Supabase env المتوقّع لغياب `.env` بالـworktree).
- **الملفات:** `modules/accounting/components/{accountStatement.logic.js, AccountStatement.jsx, TreasuryPanel.jsx, ChannelPnL.jsx}` + `screens/AccountingScreen.jsx` + `pages/AccountingDashboard.jsx` (تمرير `statementEntries`).
- **متبقٍّ (الخطة):** م2 تنظيم ذكي + بحث سريع (Command Palette + مفضّلة + تسجيل أدوات للمراجعة) · م3 مواصفة عقود/وثائق الموظفين (تنفيذ لاحق بطلب المالك) · م4 خارطة أدوات ناقصة. الخطة: `~/.claude/plans/c-users-acer-desktop-01-lowes-woolly-fern.md`.

### 🗓️ جلسة 16 يونيو 2026 (متابعة) — منظومة المالية: محافظ + كتابان + قنوات + ربح/خسارة لكل مصدر (✅ الهجرة مُطبَّقة على القاعدة الحيّة + منشور على main)
...
- **القسم 1 «المصاريف والشحن» 🚚** (`/accounting` · `AccountingScreen`) — **حساب تشغيلي لفادي ووسيم** (دورهم **مدير** فعلياً، لا محاسب → وصولهم يعمل بلا تغيير RLS):
  - نوعا القيد فقط: 🟢**استلام** + 🔴**مصروف** (`SECTION_TYPES`) — أُزيلت رواتب/سلف/تحويل. تبويبات: استلامات/مصاريف.
  - أزرار سريعة للبنود: شحن/أجور/مشتريات (مصروف) · قيمة بضاعة مباعة/توريد (استلام) + كتابة حرّة = إضافة بند جديد (نصّ حر بحقل `category`).
  - **بطاقة «الرصيد الموجود حالياً (لدى فادي ووسيم)»** = Σاستلامات − Σمصاريف (بدل لوحة الخزائن) · أُزيل زر التحويل. + جدول الوارد/الصادر لكل جهة شهرياً (`SourceBreakdown`).
  - منطق نقيّ: `operationalAccount.js` (`computeOperationalBalance`/`filterOperational`/`OP_CAT`) + بطاقة `OperationalBalanceCard` مشتركة.
- **القسم 2 «المالية العامة» 🏦** (`/ledger` · `AccountingDashboard`) = **الحساب المركزي، أدمن فقط** (nav بلا `perm` + مسار `[ADMIN]` — المديرون لم يعودوا يرونه): فلتر «الجهة/المكان» + جدول الوارد/الصادر الشهري + **بطاقة الرصيد التشغيلي + أزرار ⬇️سحب الرصيد / ⬆️توريد** (تسوية آخر الشهر → قيد واحد بفئة `تسليم/توريد الإدارة المالية`). الرواتب تُسجَّل هنا.
- **التحقّق:** `npm run build` أخضر ×3 + `test-source-breakdown.mjs` **16/16** (تجميع الجهات + الرصيد التشغيلي · الترتيب حسب **عدد الحركات** لا حجم العملة). ⚠️ **تعذّر تحقّق بصري مصادَق:** PIN الأدمن **6311 مرفوض لحساب hosam ayoub** (المعلومة القديمة بـ`[[face_manager_social]]` ربما لأدمن آخر) — لم أخمّن. المالك يتحقّق حيّاً بعد بناء Vercel.
- **ملاحظات:** قيدا التسوية (توريد دخل / تسليم مصروف) يضبطان الرصيد التشغيلي بدقّة لكنهما يضخّمان إجمالي دخل/مصروف القسم 2 (تحويل داخلي) — مقبول لمراقبة الصادر/الوارد. ملف `supabase/accounting_accountant_access.sql` **اختياري** (فادي ووسيم مديرون فلا يلزم). مرجع: `[[accounting_two_sections]]`.
...
**A) توقيت الخصم + بنية المخازن (أهم إصلاح — سبب فرق جرد مبيعات سوريا):**
- كان الخصم **عند إنشاء الطلب** → طلبات «وارد جديد/بالانتظار» غير المُسلَّمة تبقى مخصومة للأبد. الآن `syncOrderStock` (في `warehouseService.js`) مُصلِّح حالة موحّد net-aware: يخصم عند «📦 في التجهيز» فما فوق، يسترد عند الانتظار/الرجوع/الإلغاء. سوريا+تركيا. نقاط الاستدعاء: إنشاء/تعديل · تغيير حالة · ترحيل جماعي · حذف (`OrdersScreen.jsx`).
- بنية المخازن per-market (مطبّقة حيّاً على `wh_warehouses`): سوريا = مستودع كبير→مبيعات+جملة(`wholesale`) · تركيا = مستودع كبير→مبيعات. شاشة المخازن: فلتر سوق + ترتيب هرمي. SQL: `supabase/warehouse_phase3_structure.sql`.
- صُحّحت 11 «حجز شبحي» قديم (29 قطعة رجعت). **المالك يكمل جرد فعلي يدوي** للأرقام النهائية عبر «± جرد».

**B) مزامنة حالة سوريا — السبب كان realtime مطفأ:** الجدول→القاعدة يعمل أصلاً (118 تغيير source=sheet). فُعّل `ALTER PUBLICATION supabase_realtime ADD TABLE orders` (حيّ) → انعكاس فوري + `refreshStatuses` احتياطي (focus + كل 30s). ℹ️ تغيير الحالة من التطبيق لا يُكتب بالجدول (قرار سابق: الجدول مالك الحالة).
...
**أ) الهوية البصرية (brand) — مصدر واحد:**
- `src/data/brand.js` = مصدر هوية موحّد (ألوان #C9A646/أبيض/أسود · خطوط Playfair/El Messiri/Tajawal · معلومات الشركة القانونية: LOWES PROFESYONEL İÇ VE DIŞ TİCARET LİMİTED ŞİRKETİ · سجل 1120333 · العنوان · info@lowesprofesyonel.com · lowesprofesyonel.com · واتساب +90 551 817 77 98 · المفوّض **Amany Alkshky**).
- **الختم الرسمي:** `public/brand/lowes-seal.png` (من المالك) → أيقونة التطبيق (PWA 192/512 + favicon) + رأس/ختم الفاتورة والسند والراتب. اسم التطبيق = «LOWE'S Professional».
- طُبّقت الهوية على: **سند القبض/الصرف** · **فاتورة الطلب** (قالب docx: عميل/جدول منتجات/مجموع/توقيع/ختم/تذييل) · **قسيمة الراتب** · شاشة الدخول + الهيدر · محتوى السوشال (صوت البراند الرسمي) · لوزي (تعرف الهوية والنبرة) · صفحة `/brand` (كيت للفريق). مرجع ذاكرة: `[[brand-identity]]`.

**ب) ثيم هوية البراند (B) — أسود/أبيض/ذهبي:**
- `theme.css`: `--color-navy`=`#1c1628` (أسود-فحمي، كان plum) · `--color-teal`+`--color-gold`=`#C9A646` (الذهبي). النص على كل التعبئات الذهبية صار غامقاً (47 ملف/145 موضع) · الرؤوس المتدرّجة (15 ملف) → أسود صلب. **للتراجع:** navy→`45 27 78`، teal→`13 115 119`. الرؤوس **سوداء** لا بيضاء (تفسير آمن — لو المالك بدّها بيضاء = تعديل لاحق).

**ج) تنظيم UX:** بنر واحد كحدّ أقصى (Install>Push عبر uiStore) · الرئيسية بعناوين أقسام + «تحفيز اليوم» يُحفظ إغلاقه · **مكوّن `<Tabs>` موحّد** (`components/ui/Tabs.jsx`) على 5 شاشات (الطلبات/المحاسبة/المبيعات/الليدجر/الإنجازات).

**د) الأرباح:** منظومة `/profitability` كانت جاهزة لكن التكاليف=0 → أضفت **محرّر تكاليف جماعي** (BulkEconEditor). **المالك سيُدخل التكاليف يدوياً.** مرجع: `[[profitability-costs]]`.

**هـ) 7 إصلاحات (الجلسة الأخيرة):**
1. المفوّض → Amany Alkshky.
2. **فشل الفاتورة:** أُزيل `crossOrigin` من صور الختم (كان يكسر html-to-image).
3. **المحادثة:** تقفز للأحدث فوراً عند فتح الغرفة (auto + إعادات للصور) بدل البقاء على الأقدم.
4. **حذف الحملات:** حذف الأبناء أولاً (ad_daily_logs→campaign_ads) لتجاوز FK.
5. **الوردية الليلية تُحسب يومين:** الخروج يُنسب ليوم الدخول + الوردية المفتوحة تمتدّ عبر الأيام (`openShiftDate`) → يوم واحد + خروج متاح بعد منتصف الليل. (`[[attendance-shift-model]]`، going-forward فقط).
6. **تنظيف:** أُزيلت بقايا الموزعين/المناديب/MLM من AdminUsersScreen (كانت ميتة). مخازن «مناديب» تُركت (هرمية داخلية).
7. **سلاسة الجوال:** أُزيل `backdrop-blur` من الهيدر/الشريط السفلي (أثقل jank) + زخم iOS بـbody.

**⏳ متبقّي/للمتابعة لاحقاً (طلب المالك «نكمل لاحقاً»):**
- إدخال تكاليف المنتجات (المالك) لتفعيل تقارير الربح.
- تحقّق حيّ على الإنتاج لكل ما سبق (لم أتمكّن من معاينة بمصادقة — الـworktree بلا env).
- خيارات اختيارية: رؤوس بيضاء بدل سوداء للثيم · إزالة مخازن «مناديب» لو غير مستخدمة · تصحيح بيانات الحضور التاريخية المزدوجة · virtualization للقوائم الطويلة · إثراء الأدلة بمحتوى أعمق · (يورتيتشي/سوريا المؤجّلة من 11 يونيو بالأسفل).

### 📖 نظام الدليل المرجعي + تعليم لوزي — مصدر واحد (11 يونيو · جلسة سابقة)
**مشكلة محلولة:** محتوى «كيف تستخدم التطبيق» كان بمكانين يدويّين (`guides.js` + نص داخل `ai-assistant`)؛ كل مشروع جديد لازمه تحديث الاثنين. **الآن مصدر واحد** يغذّي ثلاثتها.
- **الجدول `app_guides`** (Supabase، منشور حيّاً + مزروع 17 دليل): `key/section_key/title/icon/why/steps[]/tips[]/routes[]/permission/sort_order/is_published`. RLS مفتوح + GRANT anon/authenticated. ملف: `supabase/app_guides.sql` (idempotent).
- **الواجهة:** شاشة **`/guide` «📖 دليل التطبيق»** (مقسّمة لأقسام + بحث + مفلترة بصلاحية المستخدم، بالقائمة للجميع) · زر **❓** بالهيدر (سياقي، نفس المصدر) · خدمة `src/services/guidesService.js` (+ دوال نقيّة في `guidesLogic.js`).
- **لوحة التحرير:** **`/admin/guides` «إدارة الأدلة»** (CRUD، صلاحية `MANAGE_GUIDES` = admin+manager). `guides.js` صار stub مُرحَّل.
- **لوزي:** `ai-assistant/index.ts` يبني كتلة الدليل **ديناميكياً** من `app_guides` (`formatGuidesForPrompt`) — أي دليل يُضاف باللوحة تعرفه لوزي فوراً.
- **🆕 عملية «أضفنا مشروع جديد» (الحلّ الذي طلبه المالك):** Admin → **إدارة الأدلة** → «+ دليل جديد» → اختر القسم/العنوان/الخطوات/الصلاحية → احفظ. يظهر فوراً في `/guide` لأصحاب الصلاحية **ولوزي تعرفه** — بلا كود ولا نشر.
- **مرجع:** spec `docs/superpowers/specs/2026-06-11-app-guide-system-design.md` · plan `docs/superpowers/plans/2026-06-11-app-guide-system.md`. تحقّق: تستات نقيّة 8/8 + لوزي 5/5 + build أخضر + الجدول 17 صف (REST). تقنية تنفيذ DDL: نسخ الملف للحافظة (PowerShell Set-Clipboard) ثم `clipboard.readText()` في الصفحة → `monaco setValue` → Run (تجاوز فشل لصق Ctrl+V الاصطناعي).
