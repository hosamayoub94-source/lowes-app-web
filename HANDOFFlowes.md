...
### 🗓️ جلسة 27 يوليو 2026 — لوزي معطّلة بالكامل + بُغ استدعاء أدوات انعكاسي + أسلوب استشاري + مراجعة رسائل + إصلاح موبايل ✅ build ✓ + push ✓ + Edge Functions منشورة
...
**🎭 تدريب اعتراضات (Roleplay) بالطلب الصريح (جديد):**
...
- ⚠️ **درس تقني:** أول نشر فشل ببناء الدالة — استخدمت backticks حرفية (`` ` ``) داخل نص البرومبت لعمل وسم بصيغة كود، لكن البرومبت كله أصلاً string بـtemplate literal محاط بـbackticks، فتصادمت. الحل: تجنّب أي backtick حرفي داخل نصوص البرومبت المُضافة لـ`buildSystemPrompt` — استخدمي **bold** بدلها.
