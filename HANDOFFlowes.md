...
- بديل موثوق لـ"اطلب من المالك ينفّذ SQL بالـEditor" — ينفّذ SQL مباشرة عبر Management API **بدون حاجة لكلمة مرور DB**، طالما CLI مسجّل دخول (`supabase login`) والمشروع مربوط (`supabase link`). استُخدم هالجلسة لتنظيف سجلات اختبار CRM بعد التحقّق الحي.
