...
**🐛 المشكلة المفتوحة #1 (الأهم) — كاش PWA يخدم نسخة قديمة:**
- العَرَض: قائمة الأدوار بشاشة «إضافة/تعديل موظف» تعرض **6 أدوار قديمة فقط** (موظف/مدير/مدير مبيعات/مدير سوشيال/ميديا باير/أدمن) — ناقصها مندوب/مسوّقة/مشرفة/محاسب/HR... رغم إنّ المالك يريدها.
- التشخيص (مؤكّد): **الحزمة الرئيسية المنشورة `index-*.js` فيها كل الأدوار فعلاً** (تأكّدنا بـfetch حيّ: تحتوي «مندوب ميداني»). chunk `AdminUsersScreen-*.js` يستورد ROLE_LABELS من الحزمة الرئيسية (لا يضمّنها). فالكود صحيح 100%.
- المُجرّب بلا فائدة: Redeploy نظيف بدون كاش على Vercel + unregister service worker + caches.delete + Ctrl+Shift+R عدة مرات. القائمة بقيت 6.
- **السبب الأرجح:** `vite-plugin-pwa` بوضع `registerType:'prompt'` (الافتراضي) → الـSW القديم يبقى يخدم الأصول القديمة حتى يُقبل التحديث. **الحل المقترح للمحادثة الجديدة:** غيّر إلى `registerType:'autoUpdate'` في `vite.config.js` (إعداد VitePWA) + `skipWaiting`/`clientsClaim`، أو أضف منطق تحديث SW فوري. هذا غالباً سبب **كل** شكاوى «النسخة القديمة» طوال الجلسة.
- مؤقتاً: يظهر صحيحاً في **نافذة Incognito / متصفّح نظيف**.
