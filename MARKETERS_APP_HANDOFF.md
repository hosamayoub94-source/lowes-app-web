...
- **9 يونيو 2026 — ✅ تجهيز auth + تحقّق RLS حيّ + تست عمولة شامل:**
  - **تست العمولة الشامل (كل المسارات):** field_rep junior=$8/pro=$10 · marketer bronze=$35/diamond=$50 · online يُتجاوز (لا دفتر + commission_locked) · idempotent (لا احتساب مزدوج). كلها ✅ ببيانات نُظّفت.
  - **تجهيز auth:** `node scripts/provision-auth-users.mjs` — 42 كانوا مجهّزين، أُنشئ 6 جدد (منهم حساب «test» field_rep). فشل واحد: **Fatima Ayoub** (إيميل auth مسجّل بـid مختلف — يتيم؛ غير-شبكة fail-open، لكن يستحق تنظيفاً لاحقاً).
  - **🔒 تحقّق RLS حيّ (تسجيل دخول حقيقي كالمندوب «test»):** `auth.uid == profiles.id` ✓؛ استعلام «كل الطلبات» بلا فلتر **رجع 0 من 31,782** + `crm_clients`=0 → **العزل فعّال على مستوى الـDB، ليس الواجهة فقط**. الأونلاين/الإدارة (fail-open) ما زالوا يرون الكل.
  - **النتيجة:** عزل بيانات الشركة صار **دفاعاً متعمّقاً** (UI + client-side + RLS فعّال للمندوبين المجهّزين). lint نظيف، build أخضر.
